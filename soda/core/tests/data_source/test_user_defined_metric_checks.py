    ...
    scan.add_sodacl_yaml_str(
      ...
      checks for {table_name}:
        - avg_surface between 1068 and 1069:
            avg_surface expression: AVG(product_size * distance)
        - ones(sizeTxt):
...
def test_user_defined_table_expression_metric_check_with_variables(data_source_fixture: DataSourceFixture):
    ...
    scan.add_sodacl_yaml_str(
        f"""
          checks for {table_name}:
            - avg_surface between 1068 and 1069:
                avg_surface expression: AVG(product_size * ${{dist}})
        """
    )
    scan.execute()
    ...
    scan.add_sodacl_yaml_str(
        f"""
          checks:
            - avg_surface between 1068 and 1069:
                avg_surface query: |
                  SELECT AVG(product_size * distance) as avg_surface
    ...
    scan.add_sodacl_yaml_str(
        f"""
              checks:
                - avg_surface between 1068 and 1069:
                    avg_surface query: |
                      SELECT AVG(product_size * ${{dist}}) as avg_surface
                      FROM {qualified_table_name}
            """
    )
    scan.execute()
